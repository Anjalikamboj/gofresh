    ...
    <div className="space-y-6">
      ...
      {orders.length === 0 ? (
        <div className="bg-card rounded-2xl border border-border p-12 text-center">
          ...
          <p className="text-muted-foreground">Orders will appear here once customers create subscriptions</p>
        </div>
      ) : (
              ...
              <tbody>
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-mono text-sm">{order.user_id?.slice(0, 8)}...</td>
                    <td className="px-6 py-4">
                      {order.scheduled_for ? new Date(order.scheduled_for).toLocaleDateString() : 'N/A'}
                    </td>
                    <td className="px-6 py-4 font-medium">₹{order.total_amount?.toFixed(2) || '0.00'}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {getStatusIcon(order.status)}
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${getStatusBadgeClass(order.status)}`}>
                          {order.status}
                        </span>
                      </div>
                    </td>

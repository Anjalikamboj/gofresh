...
import { Settings, Pencil, Save, X, Plus, Play, AlertCircle } from 'lucide-react';
import API from '../api';
...
function AdminPage() {
  ...
  const handleRunScheduler = async () => {
    try {
      ...
      alert('Scheduler triggered successfully! Check the Orders page.');
    } catch (err) {
      setError(err.message);
    }
  };
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-secondary text-foreground px-4 py-2 rounded-xl font-medium transition-colors hover:bg-secondary/80"
            data-testid="run-scheduler-button"
          >
